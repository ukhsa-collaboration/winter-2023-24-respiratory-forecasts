run_gtrends <- function(
  ...
  transformed_data <- .data |>
    # impute
    dplyr::group_by(icb_name) |> # was trust_code for covid; replaced throughout
    dplyr::arrange(date) |>
    ...
    dplyr::mutate(dplyr::across(dplyr::starts_with("gt_"),
                                \(.) tidyr::replace_na(., 0))) |>
    # move the target & time-variable-pop forward by the forecast horizon length
    ...
    dplyr::mutate(
      dplyr::across(dplyr::starts_with("gt_"),
  ...
  train_test_data <- tidyr::expand_grid(
    "date" = seq(min(train_data$date), max(train_data$date) + forecast_horizon,
                 by = "day"),
    # spatial identifier
    ...
    dplyr::left_join(transformed_data, dplyr::join_by(icb_name, date)) |>
    dplyr::group_by(icb_name) |>
    dplyr::arrange(date) |>
    tidyr::fill(nhs_region_name, population) |> # icb_name, shouldn't need now
    dplyr::ungroup() |>
    # need factor
    dplyr::mutate(nhs_region_name = as.factor(nhs_region_name))

  # Select all included terms:
  indicators <- colnames(train_data) |>
    stringr::str_subset("^gt_") |>
    stringr::str_subset("_lag_\\d$", negate = TRUE)
  ...
  penalise_variables <- model.matrix(reformulate(time_series_variables_pen),
                                     data = train_data)
  penalise_variables_test <- model.matrix(reformulate(time_series_variables_pen),
  ...
  model <- mgcv::bam(
    formula = target ~ -1 + penalise_variables +
      ...
      s(wday, icb_name, nhs_region_name, bs = "re") +
  ...
  fits_out <- intervals$generate_samples(
    .model = model,
    .data = dplyr::mutate(train_test_data,
                          "penalise_variables" = penalise_variables_test),
