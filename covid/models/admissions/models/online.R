run_online <- function(.data,
  ...
  transformed_data <- .data %>%
    ...
    dplyr::arrange(date) %>%
    dplyr::mutate(dplyr::across(dplyr::starts_with("online_"), ~ zoo::na.locf(., na.rm = FALSE))) %>% # forward the last NA value forward
    dplyr::ungroup() %>%
    ...
    # dplyr::mutate(across(starts_with("calls_"), ~ loess(. ~ date_numeric, span = 0.6)$fitted)) %>%
    dplyr::mutate(dplyr::across(dplyr::starts_with("online_"), ~ scales::rescale(.))) %>%
    # not sure about this one
    # tidyr::complete(tidyr::expand_grid(date = seq(min(.data$date), max_date, by = "day"))) %>%
    dplyr::arrange(date) %>%
    # add lags
    dplyr::mutate(
      dplyr::across(
  ...
  indicators <- c(
    # "online_ambo_0_19",
    # "online_clinic_0_19",
    "online_self_care_0_19",
    # "online_ambo_20_59",
    # "online_clinic_20_59",
    "online_self_care_20_59",
    "online_ambo_60_plus",
    "online_clinic_60_plus",
    "online_self_care_60_plus"
  )
