run_ets <- function(.data,
  ...
  ets_training_data <- .data |>
    dplyr::summarise(
      # only sum population for trusts with non-missing target
      population = ifelse(all(is.na(target)), NA, sum(population[!is.na(target)], na.rm = TRUE)),
      # if all trusts in an ICB have NA, ICB should have NA
      dplyr::across(where(is.numeric) & !population,
        ~ ifelse(all(is.na(.)), NA, sum(., na.rm = TRUE))),
      .by = c(date, icb_name, nhs_region_name))
